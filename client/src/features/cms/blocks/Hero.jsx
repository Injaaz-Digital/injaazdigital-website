...
import HeroVisualClient from '@/features/cms/components/HeroVisualClient';
...
function CmsHero({ block = {}, locale = 'en' }) {
  ...
  return (
    <section className="relative isolate overflow-hidden pb-14 pt-[calc(var(--header-offset)+1.5rem)] max-sm:pb-8 max-sm:pt-[calc(var(--header-offset)+0.5rem)] sm:pb-16 lg:pb-24 lg:pt-[calc(var(--header-offset)+3rem)]">
      <HeroAtmosphere />
      ...
      <div className="layout-container--hero relative">
        ...
        >
          <div className={`${isCentered ? '' : 'max-w-[42rem] max-lg:mx-auto'} order-2 text-center lg:order-1 lg:text-start`}>
            ...
            <div
              className={`mt-5 flex flex-wrap justify-center gap-3 max-sm:flex-col max-sm:[&>*]:w-full sm:[&>*]:w-auto lg:mt-8 ${
                isCentered ? 'lg:justify-center' : 'lg:justify-start'
              }`}
            >
              {primaryCta?.label ? (
                <CmsLinkButton link={{ ...primaryCta, style: 'primary' }} size="lg" />
              ) : null}
              {secondaryCta?.label ? (
                <CmsLinkButton link={{ ...secondaryCta, style: 'secondary' }} size="lg" />
              ) : null}
            </div>
          </div>
...
export { CmsHero, HeroSignalCanvas, HeroAtmosphere, HeroVisualClient };
