...
export default function PageHeroBlock({ block, onNavigate }) {
  const keyword = asText(block?.imageKeyword) || 'premium-digital-studio';

  if (block?.variant === 'editorial') {
    return (
      <section className="hero-top-spacing relative isolate flex items-end overflow-hidden bg-[#f6f9fb] pb-16 pt-36 text-[#0a2546] sm:pb-20 sm:pt-44 lg:pb-24">
  ...
  return (
    <div className="w-full max-w-full overflow-x-hidden">
      <section className="relative isolate flex min-h-[clamp(32rem,74svh,48rem)] items-center overflow-hidden pt-[calc(var(--header-offset)+var(--header-height)*1.5)] pb-[var(--header-offset)] text-[#0a2546] max-sm:min-h-[clamp(26rem,68svh,40rem)] max-sm:pt-[calc(var(--header-offset)+var(--header-height)*1.5)] max-sm:pb-[calc(var(--header-offset)-0.9rem)]">
        <HeroAtmosphere />
