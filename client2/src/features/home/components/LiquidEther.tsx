...
let liquidEtherCommon: any = null;
let liquidEtherMouse: any = null;
...
class ShaderPass {
  ...
  update(..._args: any[]) {
    if (!liquidEtherCommon?.renderer || !this.scene || !this.camera) return;
    liquidEtherCommon.renderer.setRenderTarget(this.props.output || null);
    liquidEtherCommon.renderer.render(this.scene, this.camera);
    liquidEtherCommon.renderer.setRenderTarget(null);
  }
}
...
class Simulation {
  ...
  calcSize() {
    const common = liquidEtherCommon;
    if (!common) return;
...
class Output {
  ...
  render() {
    if (!liquidEtherCommon?.renderer) return;
    liquidEtherCommon.renderer.setRenderTarget(null);
    liquidEtherCommon.renderer.render(this.scene, this.camera);
  }
  update() {
...
class WebGLManager implements LiquidEtherWebGL {
  ...
  constructor(props: any) {
    ...
    liquidEtherCommon = common;
    liquidEtherMouse = mouse;
  ...
  init() {
    if (!liquidEtherCommon?.renderer) return;
    this.props.$wrapper.prepend(liquidEtherCommon.renderer.domElement);
    this.output = new Output(this.props.paletteTex, this.props.bgVec4);
  }
  resize() {
    liquidEtherCommon?.resize();
    this.output.resize();
  }
  render() {
    if (this.autoDriver) this.autoDriver.update();
    liquidEtherMouse?.update();
    liquidEtherCommon?.update();
    this.output.update();
  }
  loop() {
  ...
  dispose() {
    try {
      ...
      liquidEtherMouse?.dispose();
      if (liquidEtherCommon?.renderer) {
        const canvas = liquidEtherCommon.renderer.domElement;
        if (canvas && canvas.parentNode) canvas.parentNode.removeChild(canvas);
        liquidEtherCommon.renderer.dispose();
        liquidEtherCommon.renderer.forceContextLoss();
      }
      liquidEtherCommon = null;
      liquidEtherMouse = null;
    } catch {
      /* noop */
    }
  }
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    const webglContext =
      probeCanvas.getContext('webgl2', { alpha: true }) ||
      probeCanvas.getContext('webgl', { alpha: true }) ||
      probeCanvas.getContext('experimental-webgl', { alpha: true });
